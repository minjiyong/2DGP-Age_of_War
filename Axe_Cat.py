...
class Attack:
    ...
    def draw(unit):
        if int(unit.frame) == 0 or int(unit.frame) == 1:
            unit.image.clip_composite_draw(int(unit.frame) * 78, 161, 78, 120, 0, 'h', unit.x, unit.y + 14, 52, 80)
        elif int(unit.frame) == 2 or int(unit.frame) == 3:
            unit.image.clip_composite_draw(151 + int(unit.frame - 2) * 113, 161, 113, 120, 0, 'h', unit.x, unit.y + 14, 78, 80)
        pass
...
class AutoRun:
    ...
    @staticmethod
    def draw(unit):
        unit.image.clip_composite_draw(int(unit.frame) * 105, 299, 105, 81, 0, 'h', unit.x, unit.y, 70, 54)
        pass
...
class Axe_Cat:
    ...
    def get_bb(self):
        return self.x-37, self.y-20, self.x+10, self.y+ 20

    def get_attack_bb(self):
        return self.x + 10, self.y - 20, self.x + 10 + self.range, self.y + 10
