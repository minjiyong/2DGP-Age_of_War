...
class Attack:
    ...
    def draw(unit):
        if int(unit.frame) == 0 or int(unit.frame) == 1:
            unit.image.clip_composite_draw(int(unit.frame) * 57, 40, 57, 59, 0, 'h', unit.x, unit.y, 57, 59)
        if int(unit.frame) == 2 or int(unit.frame) == 3:
            unit.image.clip_composite_draw(113 + (int(unit.frame) - 2) * 76, 40, 76, 59, 0, 'h', unit.x, unit.y, 76, 59)
        pass
...
class AutoRun:
    ...
    def draw(unit):
        if int(unit.frame) == 0 or int(unit.frame) == 1:
            unit.image.clip_composite_draw(int(unit.frame) * 60, 117, 60, 55, 0, 'h', unit.x, unit.y, 60, 55)
        if int(unit.frame) == 2:
            unit.image.clip_composite_draw(122, 117, 64, 55, 0, 'h', unit.x, unit.y, 64, 55)
        if int(unit.frame) == 3:
            unit.image.clip_composite_draw(187, 117, 71, 55, 0, 'h', unit.x, unit.y, 71, 55)
        pass
...
class Snake:
    ...
    def __init__(self):
        if self.image == None:
            self.image = load_image('Resource/Units_Enemy/Mobile - The Battle Cats - Snache.png')
    ...
    def get_attack_bb(self):
        return self.x - 30 - self.range, self.y - 20, self.x - 30, self.y + 10
